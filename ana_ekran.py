    ...
    try:
        ...
        def hsp(r):
            try:
                ...
                if "Saatlik" in str(r['Tür']): return 0, float(round((d-b).seconds/3600, 1))
                return int((d-b).days), 0
            except: return 0, 0
        df[['G', 'S']] = df.apply(lambda r: pd.Series(hsp(r)), axis=1)
...
if m == "⬇️ PERSONEL":
    ...
    if ps == "1234":
        ...
        with t2:
            if not df.empty:
                sl = st.selectbox("Personel Seç", sorted(df['Ad Soyad'].unique()))
