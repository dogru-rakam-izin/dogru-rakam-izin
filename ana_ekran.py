...
IZIN_LISTESI = ["Yıllık İzin", "Mazeret İzni", "Sağlık Raporu", "Saatlik İzin", "Ücretsiz İzin", "Evlilik İzni", "Vefat İzni", "Babalık İzni", "Eğitim"]
        ...
        def sure_ayristir(row):
            try:
                if "Saatlik" in str(row['Tür']):
                    f = "%d/%m/%Y %H:%M"
                    b, d = datetime.strptime(str(row['Başlangıç']), f), datetime.strptime(str(row['Dönüş']), f)
                    return 0, float(round((d - b).seconds / 3600, 1))
                else:
                    f = "%d/%m/%Y"
                    b, d = datetime.strptime(str(row['Başlangıç']), f), datetime.strptime(str(row['Dönüş']), f)
                    return int((d - b).days), 0
            except: return 0, 0
...
if menu == "⬇️ PERSONEL":
    ...
    with st.form("p_form"):
        ...
        with f2:
            if tip == "Saatlik":
                ...
                bas, bit = f"{tar.strftime('%d/%m/%Y')} {s1.strftime('%H:%M')}", f"{tar.strftime('%d/%m/%Y')} {s2.strftime('%H:%M')}"
            else:
                don = st.date_input("Dönüş")
                bas, bit = tar.strftime('%d/%m/%Y'), don.strftime('%d/%m/%Y')
        if st.form_submit_button("GÖNDER"):
            if ad and tc:
                dat = {"tarih": datetime.now().strftime("%d/%m/%Y"), "tc": str(tc), "ad": ad, "brans": "Personel", "tur": f"{tur} ({tip})", "bas": bas, "bit": bit}
                requests.post(APPS_SCRIPT_URL, data=json.dumps(dat))
    ...
    if sifre == "1234":
        ...
        with t1:
            if not df.empty:
                al = sorted(df['Ay_Ismi'].dropna().unique(), reverse=
