    ...
    if st.sidebar.text_input("Şifre", type="password") == "2020":
        ...
        with t[0]: # Karne
            if not df.empty and 'Ay' in df.columns:
                ...
                st.download_button(label="📥 Karneyi Excel İndir", data=csv, file_name=f"karne_{ay_secim}.csv")
        ...
        with t[5]: # Liste & Sil
            if not df.empty:
                ...
                st.divider()
                sil_id = st.number_input("Silinecek Satır Numarası (SİLME_ID):", min_value=2, step=1)
                if st.button("❌ SEÇİLİ KAYDI SİSTEMDEN KALDIR"):
                    # BURASI ÖNEMLİ: URL'ye "islem: sil" gönderiyoruz
                    resp = requests.post(URL, data=json.dumps({"islem": "sil", "satir": int(sil_id)}))
                    if resp.status_code == 200:
                        st.success(f"Satır {sil_id} silindi. Güncel hali görmek için lütfen sayfayı yenileyin.")
                        st.balloons()
                    else:
                        st.error("Bir hata oluştu, silinemedi.")
