...
st.markdown(f"<style>[data-testid='stSidebarNav'] {{ background-image: url({LOGO_URL}); background-repeat: no-repeat; padding-top: 140px; background-position: center 20px; background-size: 150px auto; }} .main-title {{ color: #CC0000; font-size: 38px; font-weight: bold; }} div.stButton > button {{ background-color: #25D366 !important; color: white !important; border-radius: 8px; font-weight: bold; }} </style>", unsafe_allow_html=True)
...
if m == "👤 PERSONEL GİRİŞİ":
    ...
    with st.expander("📝 Başvuru Formu", expanded=True):
        with st.form("p_f"):
            t1, t2 = st.selectbox("Tür", IZ[:-1]), st.date_input("İzin Günü") # Son Tür (Geç Kalma) personelde gözükmesin
            if tp == "Saatlik":
    ...
    if st.sidebar.text_input("Şifre", type="password") == "1234":
        ...
        with t[3]: # YENİ: Geç Kalma Girişi
            ...
            with st.form("g_f"):
                ...
                if st.form_submit_button("GEÇ KALMA KAYDI OLUŞTUR"):
                    # Excel'e 1 saatlik izin gibi ama "Geç Kalma" türünde kaydeder
                    g_bas = f"{g_tar.strftime(F_TARIH)} 09:00" # Varsayılan sabah saati
                    # Dönüş saati olarak başlangıç + dakika eklemesi (basit mantık)
                    ...
                    st.session_state['wa_g'] = f"⏰ *GEÇ KALMA BİLD
