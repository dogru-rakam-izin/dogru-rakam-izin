    ...
    with st.form("p_f"):
        ...
        if kaydet:
            if ad:
                ...
                st.success("Kayıt Başarılı. WhatsApp butonunu kullanabilirsiniz.")
            else:
    ...
    if st.sidebar.text_input("Şifre", type="password") == "2020":
        ...
        with t[0]: # Karne
            if not df.empty and 'Ay' in df.columns:
                ...
                # Hatalı olan satırları temizleyip doğrudan sayısal veriyi gösteriyoruz
        ...
        with t[3]: # Geç Kalma
            ...
            with st.form("g_f"):
                ...
                g_dak = st.slider("Gecikme (Dakika)", 1, 60, 15)
                if st.form_submit_button("GEÇ KALMA KAYDET"):
                    ...
                    st.success("Geç kalma kaydı yapıldı.")

        with t[4]: # Yıllık İzin
            py = st.selectbox("Hakediş Sorgula", p_listesi, key="yil_sor")
        ...
        with t[5]: # Liste & Sil
            if not df.empty:
                ...
                st.download_button(label="📥 Tüm Listeyi Excel Olarak İndir", data=csv_full, file_name="izin_yedek.csv")
                st.divider()
                sid = st.number_input("Silinecek Satır No:", min_value=2, step=1)
                if st.button("❌ SEÇİLİ KAYDI SİL"):
                    requests.post(URL, data=json.dumps({"islem": "sil", "satir": int(sid)}))
                    st.success(f"{sid} numaralı satır silindi. Güncelleme için sayfayı yenileyin.")
    else: st.warning("Yönetici şifresi gerekli.")
