...
# --- GRUP AYARI ---
# WhatsApp grup davet linkinizi buraya yapıştırın
GRUP_LINKI = "https://chat.whatsapp.com/E6AvtPY5GOV7W4xrRsFbNe?mode=gi_t"
...
if m == "👤 PERSONEL GİRİŞİ":
    ...
    with st.expander("📝 Başvuru Formu", expanded=True):
        with st.form("p_f"):
            t1, t2 = st.selectbox("Tür", IZ), st.date_input("İzin Günü")
            if tp == "Saatlik":
            ...
            if st.form_submit_button("SİSTEME KAYDET"):
                if ad:
                    ...
                    st.success("Kayıt veritabanına eklendi. Şimdi aşağıdan gruba bildirin.")
    ...
    if st.sidebar.text_input("Şifre", type="password") == "2020":
        ...
        with t[2]: # Manuel Giriş
            with st.form("m_f"):
                ...
                m_tr, m_tarih = st.selectbox("Tür", IZ), st.date_input("Tarih")
                ...
                if st.form_submit_button("MANUEL KAYDI EKLE"):
                    ...
                    st.session_state['wa_m'] = f"✅ *MANUEL İZİN GİRİŞİ*\n👤 *Personel:* {m_ad}\n📋 *Tür:* {m_tr}\n📅 *Tarih:* {mb}\n\n_Bu kayıt yönetici tarafından sisteme işlenmiştir._"
        ...
        # Diğer sekmeler standart fonksiyonlarını korur
        with t[0]: # Karne
            if not df.empty and 'Ay' in df.columns:
                ay_list = sorted(df['Ay'].dropna().unique(), reverse=True); ay = st.selectbox("Ay Seç", ay_list)
